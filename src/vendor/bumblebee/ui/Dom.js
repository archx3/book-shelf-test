...
let Bu = require('../base/Utils'),
    Ba = require('../base/Array'),
    Bo = require('../base/Object'),
    Bs = require('../base/string/String');
...
var Dom = {
  ...
  {
    return this.matches(el, 'input,[contenteditable]') ||
           this.matches(el, 'textarea,[contenteditable]');
  },
        ...
        else
        {
          ...
          else
          {
            var clockOut2 = setTimeout(function ()
                                       {
                                         this.css(windowEl, {display : "none"});
        ...
        var clockOut = setTimeout(function ()
                                  {
                                    ...
                                    this.css(windowEl, {display : "block"});
                                  }, delayVal);
      }
      else
      {
  ...
  $_GET : function (variableName)
  {
    try
    {
      ...
      if (variableName in getVars && Bu.defined(getVars[variableName]))
      {
        ...
        return val.replace("+", " ").replace("%20", " ").trim();
      }
  ...
  $_SET : function (variableName, newValue)
  {
    try
    {
      ...
      if (variableName in getVars && Bu.defined(getVars[variableName]))
      {
        ...
        return val.replace(getVars[variableName], newValue).trim();
      }
  ...
  {
    var self = this;
    if (Bu.isString(elRule) && el)
    {
      return Bu.pInt(el.style[elRule].toString().replace(/px|%|pt|em/gi, ""));
    }

    if (elRule.toString().indexOf(/px|%|pt|em/gi) > -1)
    {
      return Bu.pInt(elRule.toString().replace(/px|%|pt|em/gi, ""));
    }
    return Bu.pInt(elRule.toString());
  },
  ...
  createItems : function (items, isSub)
  {
    ...
    Bo.forEach(items, function (item)
    {
      ...
      { // An array will return 'object'
        ...
        html += this.createItems(item.sub, true);
      }
      else
      {
  ...
  index        : function (el, selector)
  {
    ...
    {
      if ((el.nodeName.toUpperCase() !== 'TEMPLATE') && (selector === '>*' || this.matches(el, selector)))
      {
        index++;
      }
    }
  ...
  centerToPageHorizontally : function (el, offset = 0, useWidth)
  {
    ...
    else
    {
      if (useWidth)
      {
        horMid = (window.innerWidth / 2) - (Bu.pInt(Bu.getStyleValue('width', el)) / 2) - offset;
      }
      else
      {
  ...
  centerToPageVertically   : function (el, offset = 0, useHeight)
  {
    ...
    else
    {
      ...
      {
        console.log(offset);
        verMid -= ((Bu.pInt(Bu.getStyleValue('height', el))) / 2) + offset;
      }
      else
      {
  ...
  getOwnerDocument : function (node)
  {
    ...
    /** @type {!Document} */
    return (node.nodeType == this.NodeType.DOCUMENT ?
  ...
  setPseudoStyle : function (element, pseudoClass, prop, value)
  {
    ...
    sheet.id = sheetId;
    var className = "pseudoStyle"/* + this.UID.getNew()*/;
  ...
  {
    return Bu.isObject(obj) && obj.nodeType === this.NodeType.ELEMENT;
  },
  ...
  getChildren : function (element)
  {
    ...
    {
      return node.nodeType === this.NodeType.ELEMENT;
    });
  },
  ...
  {
    var self    = this,
        overlay = this.createEl('section', { className : 'overlay' });
    this.css(overlay, {
      zIndex          : 200,
      position        : 'absolute',
      top             : 0,
  ...
  createTable : function (doc, rows, columns, content, fillWithNBSP)
  {
    ...
    for (var i = 0; i < rows; i++)
    {
      ...
      for (var j = 0; j < columns; j++)
      {
        ...
        if (fillWithNBSP)
        {
          this.setTextContent(td, Bee.String.Unicode.NBSP);
        }
  ...
  setTextContent : function (node, text)
  {
    ...
    else if (node.nodeType === this.NodeType.TEXT)
    {
      node.data = text;
    }
    else if (node.firstChild && node.firstChild.nodeType === this.NodeType.TEXT)
    {
  ...
  setRootStyle : function (PropertyName, value)
  {
    return this.CSSSTYLE.setProperty(PropertyName, value);
  },

  getRootStyle    : function (PropertyName)
  {
    return this.CSSSTYLE.getPropertyValue(PropertyName);
  },
  surrogateButton : function ()
  {
    var sBtns = this.getElementsByAttribute("data-sbtn");
    // console.log(sBtns);
    if (sBtns)
    {
      Ba.forEach(sBtns, function (sBtn, i)
      {
        if (Bu.defined(sBtn.getAttribute("data-sbtn")) && !Bs.isEmpty(sBtn.getAttribute("data-sbtn")))
        {
          ...
          {
            let actualBtn = this.getEl('#' + e.target.getAttribute("data-sbtn"));

            if (Bu.defined(actualBtn))
            {
              if ((!this.disabled(actualBtn)))
              {
                actualBtn.click();
      ...
      /*for (i = 0, len = sBtns.length; i < len; i++)
       ...
       {
       var actualBtn = this.getEl('#' + this.getAttribute("data-sbtn"));
